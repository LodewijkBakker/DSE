...
class Satellite:
    ...
    def orbital_el_set_up(self):
        a=self.dick["Orbital parameters"]["a"]
        R,V= self.kep_2_cart(a,self.dick["Orbital parameters"]["e"],self.dick["Orbital parameters"]["i"],self.dick["Orbital parameters"]["omega_AP"],self.dick["Orbital parameters"]["omega_LAN"],self.dick["Orbital parameters"]["T"],self.dick["Orbital parameters"]["EA"],self.dick["Orbital parameters"]["t"])
        print(R,V)
    ...
    def dy_vector(self, t, Y):
        ...
        if(np.cos(sh_ang)*R_abs<R_earth and sh_ang<np.pi/2):#alternatively sin(sh_ang)
            self.shadow=True
...
if __name__ == "__main__":
    ...
    #print(LAMP.Y0)
    LAMP.orbital_el_set_up()
    """"
    sol=solve_ivp(LAMP.dy_vector,[0,10000],LAMP.Y0, max_step=0.1,atol = 1, rtol = 1)#, method="Radau"
    print(sol.y.shape)
